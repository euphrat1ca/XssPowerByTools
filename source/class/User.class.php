...
class User{
	public $userId=0,$adminLevel=0,$userName='',$email='',$avatarImg='',$signature='',$token;
	...
	/* 登录界面 */
	function ToLogin(){
		@header('Location: '.URL_ROOT.'/xss.php?do=login');
		exit;
	}
	/* 登录 */
	function Login($username='',$password='',$autoLogin=0){
		...
		if($row && $row['userPwd']==OCEncrypt($password)){
			if(MAIL_AUTH && $row['validated']==0){
				...
				$data=serialize(array('userId'=>$row['id'],'adminLevel'=>$row['adminLevel'],'userName'=>$row['userName'],'email'=>$row['email'],'avatarImg'=>$row['avatarImg'],'avatarImg_s'=>$row['avatarImg_s'],'signature'=>$row['signature']));
				$sqlValue=array(
					'userId'=>$row['id'],
					'ocKey'=>$ocKey,
					'token'=>$token,
