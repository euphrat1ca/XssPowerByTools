...
function OCEncrypt($str){
	return md5('t00ls.net-uid-7520|'.$str);
}
...
function StripStr($str){
	...
	$str=addslashes(htmlspecialchars($str,ENT_QUOTES));
...
if($turnto==URL_ROOT)
	Notice($str,$turnto.'/xss.php',3,'success',$urltitle);
else
...
if($turnto==URL_ROOT)
	Notice($str,$turnto.'/xss.php',3,'error',$urltitle);
else
...
function SendMail($to='',$subject='',$body=''){
        ...
        require_once(ROOT_PATH.'/source/class/PHPMailer.class.php');
		$db=DBConnect();
		$fromlist=$db->Dataset("SELECT * FROM ".Tb('email')." ORDER BY id dESC");
        shuffle($fromlist);
        try{
                ...
                {       
						...
						$mail->Port = 25; 								   //当邮件服务器未使用SSL的时候 端口改为25
						$mail->From = $v['euser']; 
						$mail->FromName = "饼干快递公司"; 
                        $mail->Host = $v['ehost'];    
                        $mail->Username = $v['euser'];   
                        $mail->Password = $v['epass'];                
                        $mail->CharSet = "UTF-8";
...
//发送邮件
function SendtestMail($to='',$subject='',$body='',$content=''){
        //global $mailConfig;
        require_once(ROOT_PATH.'/source/class/PHPMailer.class.php');
        try{
                    $mail= new PHPMailer();        
                    ...
                    //$mail->SMTPSecure = "ssl";						   //当邮件服务器未使用SSL的时候 注释本段
					$mail->Port = 25; 								   //当邮件服务器未使用SSL的时候 端口改为25
					$mail->From = $content['euser']; 
					$mail->FromName = "饼干快递公司"; 
                    $mail->Host = $content['ehost'];    
                    $mail->Username = $content['euser'];   
                    $mail->Password = $content['epass'];                
                    $mail->CharSet = "UTF-8";
                    $mail->AddAddress($to, $to); 
                    $mail->Subject = $subject; 
                    $mail->AltBody    = "To view the message, please use an HTML compatible email viewer!";
...
/* 短网址字符 */
function ShortUrlCode($existed=array(),$num=4){
